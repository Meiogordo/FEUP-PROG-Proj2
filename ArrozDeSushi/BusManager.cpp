...
void BusManager::displayLines() {
	for (int i = 0; i < lines.size(); i++) {
		cout << "ID: " << lines[i].ID << " Primeira e �ltima paragem: "		cout << lines[i].stops.at(0) << " ... " << lines[i].stops.at(lines[i].stops.size());
		cout << endl;
	}
}
...
bool BusManager::Load() {
	...
	while (!inputDrivers.is_open()) {
		cout << "Nome do ficheiro inv�lido!" << endl		cout << "Insira o nome do ficheiro a usar para os condutores: (exemplo: \"condutores_test.txt\")" << endl;
		...
		if (cin.eof()) {
			cin.clear();
			cin.ignore(10000, '\n');
			cout << "EOF detetado, abortando processo de cria��o de gestor de empresa de autocarros..." << en			cout << "Pressione enter para continuar..." << endl;
			cin.get();
		}
		inputDrivers.open(inputpath);
	}
	...
	/*
	cout << "Conte�do do ficheiro dos condutores:" << endl	Utilities::printVector(RawDrivers);
	cout << "Conte�do do ficheiro das linhas:" << endl	Utilities::printVector(RawLines);
	*/
