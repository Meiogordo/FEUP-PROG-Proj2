...
BusManager::BusManager() {
	...
	while (!inputLines.is_open()) {
		cout << "Nome do ficheiro inv�lido!" << endl		cout << "Insira o nome do ficheiro a usar para as linhas: (exemplo: \"linhas_test.txt\")" << endl;
		getline(cin, inputpath);
		inputLines.open(inputpath);
	}
...
bool BusManager::createNewLine() {
	...
	while (true) {
		cin >> newID;
		if (cin.fail()) {
			cout << "ID inv�lido, por favor introduza um ID v�lido (n�mero inteiro)." << 			//Clearing error flag and cin buffer
			cin.clear();
			cin.ignore(10000, '\n');
		}
		else {
